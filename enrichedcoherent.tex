...
\begin{defn}
  Given a presheaf \(X\) on \(\Theta[\C]\), two vertices \(x,y\) in \(X_0\) and a category of gadgets \(\mathcal{G}\), we define a simplicial presheaf on \(\C\) by the formula \[\mathfrak{C}^{\mathcal{G}}(X)(x,y)_c\coloneqq N\overcat{\mathcal{G}_c}{X_{x,y}}.\]  
...
\[C^\bullet_{\mathrm{cyl}}(c)\coloneqq \colim \left( V[1](\emptyset) \leftarrow \Delta^\bullet \times V[1](\emptyset) \hookrightarrow \mathscr{H}(\Delta^\bullet)\times [1](c)\right)\]
\[C^\bullet_{E}(c)\coloneqq \colim \left( V[1](\emptyset) \leftarrow E^\bullet \times V[1](\emptyset) \hookrightarrow E^\bullet\times [1](c)\right)\]
\[C^\bullet_{R}(c)\coloneqq \colim \left( \ast \leftarrow \Delta^\bullet \xrightarrow{\partial^{\bullet}} (\bullet)^\triangleright(c)\right)\]
\[C^\bullet_{L}(c)\coloneqq \colim \left( \ast \leftarrow \Delta^\bullet \xrightarrow{\partial^0} (\bullet)^\triangleleft(c)\right).\]
...
\end{proof}
\begin{defn} Given a bipointed presheaf \(X_{x,y}\) on \(\Theta[\C]\) we define the \emph{mapping object} from \(x\) to \(y\) to the simplicial presheaf obtained by taking homotopy function complexes \[\Map_X(x,y)_c\coloneqq h\psh{\Theta[\C]}_{\ast,\ast}([1](c), X).\] 
\end{defn}
...
\begin{cor} For any formal \(\C\)-quasicategory \(X\) and any pair of vertices \(x,y\), there is a natural zig-zag of weak equivalences between \(\Map_X(x,y)\) and \(\mathfrak{C}(X)(x,y)\).
\end{cor}
...
\begin{rem} This theorem is actually a really strong and surprising result because it implies that the counit map is a weak equivalence for categories enriched in any cartesian-closed left-Bousfield localization of \(\Psh_\Delta(\C)_{inj}\).  It reduces proving comparison theorems for such localizations to showing that \(\mathfrak{C}\) is a left-Quillen functor (something we already know for the horizontal Joyal model structure by Corollary \ref{horizquillen}) and homotopy-conservative.  
\end{rem}

\section{Horizontal Dwyer-Kan equivalences}
