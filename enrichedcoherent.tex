...
\section{The coherent nerve and realization}
...
\begin{lemma}\label{dsrectangle1}
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzpicture}.
	\end{center}
	This diagram is obtained by composing the diagram from the previous lemma on the right with the diagram in the proof of \cite{ds2}*{Proposition 5.8}, which we reproduce here:
	\begin{center}
		\begin{tikzpicture}
			\matrix (b) [matrix of math nodes, row sep=3em,
				column sep=2em]
	...
	The two horizontal maps in the bottom row are weak equivalences by \cite{ds2}*{Proposition 5.8}, as are the indicated vertical maps, using the fact that \(k^\ast(\mathfrak{N}\mathcal{D})_c\) is a quasicategory. By \(3\)-for-\(2\), it follows that the horizontal maps in the third row are all weak equivalences.  We therefore reduce the composite diagram to a smaller diagram:
	\begin{center}
		\begin{tikzpicture}
			\matrix (b) [matrix of math nodes, row sep=3em,
				column sep=2em]
