...
\begin{obs}\label{square2}
	...
	\begin{equation*}
		\begin{tikzpicture}
			...
			(b-1-2) edge node[auto]{\(\scriptstyle{\id \times (P_i^\lrcorner)_{i=1}^r}\)} (b-1-3) 
...
where \((h_i:\Lambda_i \to \Delta^{r_i})_{i=1}^k\) are inner horn inclusions where \(h_i\) is a morphism over \(\Delta^n\times\Delta^m\).  Let \(h^\prime_i\) denote this morphism as map in simplicial sets over \(\Delta^{r_i}\), and let 
