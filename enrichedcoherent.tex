...
We would like to give a special thanks Alexander Campbell for his invaluable help in correcting the proof of \Cref{joyalisothm}, which previously cited an incorrect result.
...
\begin{defn}\label{cishelp}
	A small regular skeletal Reedy category (also called a regular skeletal category) \(\C\) (see \cite{cisinski-book}*{8.2.3}) is called a \emph{regular Cartesian Reedy category} if it satisfies two conditions:
...
We recall in the appendix a number of important technical results of Cisinski theory, which comprises a large body of results on the construction of extremely tame model structures on presheaf categories.
\subsection{Cisinski model structures and localizers}
In what follows, we will work with a fixed small category \(\mathcal{A}\). These are stated in more generality in \cite{cisinski-book}*{Chapter 1}, but we will specialize to the case of a Cartesian cylinder functor.
\begin{defn}
...
\end{prop}
\begin{defn}
	A \emph{class of anodynes} \(\mathbf{An}\) for a separating interval \(I\) is a class of monomorphisms \(f\) that satisfies the following properties:
	\begin{itemize}
...
\end{cor}
\begin{defn}
	An \(\mathcal{A}\)-localizer \(\mathsf{W}\) is a class of maps satisfying the following axioms
	\begin{itemize}
...
\begin{rem}
	...
	The free homotopy theory on \(\mathcal{A}\) is in general radically different from the homotopy theory given by the injective model structure on simplicial presheaves, which we will see later is its regular completion.  Cisinski gives the example where \(\mathcal{A}=B\mathbb{G}\) for a simplicial group \(\mathbb{G}\).  The difference between the free homotopy theory and its regular completion in this case is the difference between equivariant homotopy theory and higher Galois theory.  That is, the free homotopy theory presents ordinary equivariant homotopy theory, while the regular completion of the free homotopy theory on \(B\mathbb{G}\) models non-abelian \(\mathbb{G}\)-representations. 
\end{rem}
\begin{defn}
...
\begin{prop}[\cite{cisinski-book}*{Proposition 2.3.27}]
	If \(\mathsf{W}\) is any accessible localizer, the functor \(p^\ast:\psh{\mathcal{A}} \to \psh{\mathcal{A}\times \Delta}\) induced by the projection \(\mathcal{A}\times \Delta \to \mathcal{A}\) is a left Quillen equivalence.  Also, by choosing a Reedy-cofibrant cosimplicial resolution \(D^\bullet\) of the terminal object \(\ast\) of \(A\), the functor 
	\[
...
\end{defn}
\begin{prop}[\cite{cisinski-book}*{Remark 3.4.14}]
...
\end{prop}
\begin{prop}[\cite{cisinski-book}*{Proposition 3.4.34}]
...
\end{defn}
\begin{prop}[\cite{cisinski-book}*{Proposition 8.1.37}]
