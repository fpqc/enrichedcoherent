...
We would like to thank Denis-Charles Cisinski for his help with axiomatizing the Eilenberg-Zilber shuffle decomposition \ref{cishelp}, Andrea Gagna for spending his time discussing these ideas with us over several long conversations as well as his crucial encouragement in writing it up. 
...
For the purposes of this paper, we will not necessarily need this level of generality, but we expect it may be useful in the future. 
...
\begin{defn} For any small regular Cartesian Reedy category \(\C\) admitting a terminal object object, we define the category of \(\C\)-cells
...
In this section, we define general corner tensors and apply them to define the corner tensor functor.
\label{cornertensorfunctoriality}
...
\begin{obs}
	...
	\[K_{\alpha,\beta}:\psh{\C}^n and \psh{\C}^m\to \psh{\C}^r\] be the functor defined by the rule
	...
	Then the \(P_i\) assemble to a map \(P_1\times \dots \times P_r\) such that \[P_1\times \dots \times P_r\circ \tau = K_{\alpha,\beta}.\]
	Then the diagram
	\begin{equation*}
		\begin{tikzpicture}
			\matrix (b) [matrix of math nodes, row sep=3em, column sep=4em]
			...
			(b-1-2) edge node[auto]{\(\scriptstyle{\id \times \prod_{i=1}^r P_i}\)} (b-1-3) 
	...
	\begin{equation*}
		\begin{tikzpicture}
			...
			(b-1-2) edge node[auto]{\(\scriptstyle{\id \times \mathbf{P}^\lrcorner}\)} (b-1-3) 
			(b-1-3)	edge node[auto]{\(\scriptstyle{\square^\lrcorner_r}\)} (b-2-3);
		\end{tikzpicture},
	\end{equation*}
where \(\mathbf{P}=\prod_{i=1}^r P_i\) and any other unexpected changes were done for typographical reasons.  
\end{obs}

Finally, we reach our destination.  As we mentioned before, this entire section is due entirely to David Oury \cite{oury}, and the following theorem is the main point of this entire Appendix:

\begin{thm}[Anodyne Theorem \cite{oury}*{3.88}] The class of horizontal anodynes is closed under corner products with monomorphisms.  In particular, if we let \[\mathscr{J}=\{\square_n^\lrcorner(\lambda^n_k, \delta^{c_1},\dots,\delta^{c_n})| \text{ for } n\geq 2, 0<k<n\}.\] Then we have
	\[\mathscr{J}\times^\lrcorner \mathscr{M} \subseteq \operatorname{Cell}(\mathscr{J}).\]
\end{thm}
\begin{proof}
\end{proof}
