...
\begin{prop}\cite{htt}*{4.2.4.7}\label{lurieprop2}
	...
	\end{itemize}
\end{prop}
\begin{proof} No significant changes to the proof of \cite{htt}*{4.2.4.7}.
\end{proof}
...
\end{prop}
\begin{proof} 
	See \cite{htt}*{5.1.3.1}.
\end{proof}
%\begin{prop} Let \(X\) be a small \(\C\)-cellular set, let \(f:X\to \M\) be an object of \(\M^{X^{\op}\), and let \(F:\M^{X^{\op} \to \M^+,\) where \(\M^+\) denotes 
%\end{prop}

\section{Representability and weighted limits and colimits}
