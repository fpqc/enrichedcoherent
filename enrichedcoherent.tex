...
\begin{defn} A small regular skeletal Reedy category \(\C\) (see \cite{cisinski-book}*{8.2.3}) is called a \emph{regular Cartesian Reedy category} if the product of two representables \(c_1 \times c_2\) in \(\psh{C}\) is regular in the sense of \cite{cisinski-book}*{8.2}.
\end{defn}
\begin{rem} We suspect that in fact, all regular skeletal Reedy categories are regular Cartesian, but the examples we care about all satisfy this stronger property, so we leave this question to another day.
\end{rem}
For the purposes of this paper, we will not necessarily need this level of generality, but we expect it may be useful in the future. In the sequel, we assume that \(\mathcal{V}=\psh{\C}\) is the category of presheaves of sets on a small regular Cartesian Reedy category \(\C\) admitting a terminal object.  

Then we give the following definition:

\begin{defn} For any small regular Cartesian Reedy category \(\C\) admitting a terminal object object, we define \(\Theta[\C]\subseteq \Delta \int \psh{\C}\) to be the full subcategory spanned by the objects of the form \([n](h_{c_1},\cdots, h_{c_n})\) for \(c_1,\cdots, c_n \in \C\), and where \(h_\bullet\) denotes the Yoneda embedding.
\end{defn}
...
In this section, we define general corner tensors and apply them to define the corner-intertwiner and corner-product.

\section{The boundary and anodyne theorems}\label{horizontal}
In this section, we demonstrate that the class of monomorphisms is exactly the class of relative cell complexes for \(\mathscr{M}\), which we will show coincides with the set of boundary maps for the regular Reedy category \(\Theta[\C]\).  We will also show that the class of horizontal inner anodynes is closed under corner products with monomorphisms.  The content of this section is a relatively straightforward generalization of the content of \cite{oury}*{3.4.4}.

\begin{prop} 
	The category \(\Theta[\C]\) is a regular skeletal Reedy category whenever \(\C\) is a regular Cartesian Reedy category.  The dimension function of this Reedy category is given by 
	\[\operatorname{dim}[n](c_1,\dots,c_n)\defeq n+\operatorname{dim}_\C c_1 + \dots + \operatorname{dim}_\C c_n,\]
	and the boundary can be computed by the formula 
	\[\partial [n](c_1,\dots,c_n)\defeq \operatorname{dom} \square^\lrcorner_n(\delta^n,\delta^{c_1},\dots,\delta^{c_n}).\]
\end{prop}
\begin{proof} 
	It follows by \cite{bergner-rezk-reedy}*{Proposition 4.4} that \(\Theta[\C]\) is normal skeletal Reedy. To prove that \(\Theta[\C]\) is regular, it suffices to show that the image of any map 
	\[(\alpha,\mathbf{f}):[n](c_1,\dots,c_n) \to [m](d_1,\dots, d_m)\]
	is representable.  It suffices by normality to show this in the case that the map is monic.  Since \(\C\) is cartesian, the image of any monomorphism \(c\to \prod_{i=j}^k d_i\) is representable, and since it is nondegenerate, and it follows immediately that the image of \(\alpha,\mathbf{f}\) is exactly \([n](c_1,\dots, c_n)\).  
	To see that our formula for the boundary is correct, let 
	\[(\alpha,\mathbf{f}):[n](c_1,\dots,c_n) \to [m](d_1,\dots, d_m)\]
	be a codimension \(1\) coface.  Then it is clear immediately that either \(n=m\) or \(n=m-1\), and therefore either \(\alpha\) is the identity or a simplicial face map.  If \(n=m\), then clearly \(\alpha, \mathbf{f}\) lands in exactly one of the \(V[m](d_1, \dots, \partial d_i, \dots, d_m).\)  If \(\alpha\) is an inner face map obtained by removing the \(k^\mathrm{th}\) vertex, then \((\alpha,\mathbf{f})\) factors through the inclusion \[V[m](d_1,\dots,d_{k-1} \times d_k,\dots, d_m) \subseteq [m](d_1,\dots,d_m)\] by a monomorphism \(\operatorname{id}, \mathbf{g}\) where \(g_i\) is either the identity or the inclusion of a shuffle \(c\to d_{k-1} \times d_k\), which is representable by the fact that \(\C\) is regular Cartesian.   
\end{proof}
\begin{cor} 
	The class \(\operatorname{Cell}(\mathscr{M})\) is exactly the the class of monomorphisms of \(\cellset\).  
\end{cor}
\begin{proof}
	This is an immediate consequence of \cite{cisinski-book}*{Proposition 8.1.37}.
\end{proof}
