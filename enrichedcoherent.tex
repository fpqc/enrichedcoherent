...
\begin{proof} 
  ...
  Notice that \(\mathscr{G}(X^{\bullet})\) is exactly the simplicial mapping space \(\operatorname{hMap}^\Delta(bullet,X)\).  For any two vertices \(x,y\) of \(X\), it suffices therefore to show that \(\operatorname{hMap}^\Delta_{\ast,\ast}(\Sigma(f),X)\) is a trivial fibration.  But \(\operatorname{hMap}_{\ast,\ast}(\Sigma(f),X_{x,y})\) is exactly \(\operatorname{hMap}(f,\Map_X(x,y)\), which is a trivial fibration if and only if \(\Map_X(x,y)\) has the right lifting property with respect to \(b \times^\lrcorner f\) where \(b\) is a simplicial boundary inclusion, which proves the claim.
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    \end{cases}. 
  \]
  The functor \(\mathbf{2}(\bullet)\) is clearly a left-Quillen functor 
  \[\Psh_\Delta(\C)_{\mathrm{inj}} \to \overcat{\ast \coprod \ast}{\Cat_{\Psh_\Delta(\C)_{\mathrm{inj}}}},\] so in particular, it preserves homotopy-colimits, and for all \(n \geq 0\) and \(c \in \C\), there is a natural weak equivalence 
  ...
  that lifts to an equivalence
  \[\mathfrak{C}(\Sigma(\Delta^n\times c) \xrightarrow{\sim} \mathbf{2}(\Delta^n\times c).\]  Since both functors preserve homotopy colimits, we see that for any simplicial presheaf \(A\) on \(\C\), the induced map 
  \[\gamma_A:\mathfrak{C}(\Sigma(A) \xrightarrow{\sim} \mathbf{2}(A).\]
