...
We begin by constructing the Yoneda embedding relative to a cartesian presentation \(\M=(\C,\setS)\):
\begin{defn}
	Let \(X\) be a presheaf on \(\Theta[\C]\), and let \(f:\mathfrak{C}(X) \xrightarrow{\sim} \mathcal{D}\) be an \(\M\)-enriched fibrant replacement.  Since \(\mathcal{D}\) is fibrant, the enriched Yoneda embedding 
	\[\mathcal{D}\hookrightarrow \Psh_\Delta(\C)^{\mathcal{D}^\op}\]
	factors through the full subcategory \((\Psh_\Delta(\C)_\M^\circ)^{\mathcal{D}^\op}\) where \(\Psh_\Delta(\C)_\M^\circ\) is the full subcategory of \(\M\)-fibrant objects.  This factorization exists since all of the \(\Hom\)-objects of \(\mathcal{D}\) are \(\M\)-fibrant.  By adjointness, we obtain a map 
	\[\mathcal{D} \times \mathcal{D}^\op \to \Psh_\Delta(\C)_\M^\circ.\]
	We also have a natural map 
	\[\mathfrak{C}(X\times X^\op) \xrightarrow{\mathfrak{C}(p_1) \times \mathfrak{C}(p_2)} \mathfrak{C}(X) \times \mathfrak{C}(X)^\op \xrightarrow{f \times f^\op} \mathcal{D}\times \mathcal{D}^\op,\]
	and composing these maps together, we obtain a map 
	\[\mathfrak{C}(X\times X^\op) \to \Psh_\Delta(\C)_\M^\circ,\]
	which yields, by adjunction,
	\[X\times X^\op \to \mathfrak{N}(\Psh_\Delta(\C)_\M^\circ).\]
	We denote \(\mathfrak{N}(\Psh_\Delta(\C)_\M^\circ)\) simply by \(\M\), and finally, we obtain \emph{the Yoneda embedding} for \(\M\)-quasicategories:
	\[X\to \M^{X^\op}.\]
\end{defn}
