...
\begin{lemma} Given a simplicial set \(S\), the category of labelings of \(S\) (that is, the fibre \((\psh{\Delta}\int \psh{\C})_S\)) is closed under finite Cartesian products. 
\end{lemma}
...
\begin{defn} Given a simplicial set \(S\) define the functor 
	\[H_S: \overcat{\psh{\Delta}}{S} \times  \left(\psh{\Delta}\int \psh{\C}\right)_S \to \psh{\Delta}\int \psh{\C}\]
	by the rule
	...
	Notice that when \(S=\Delta^n\), the fibre decomposes as \[\left(\psh{\Delta}\int \psh{\C}\right)_n\cong \psh{\C}^n.\] So we can write 
...
\begin{defn}
	...
	\[H_{\mathbf{S}}: \overcat{\psh{\Delta}}{\prod_{i=1}^n S_i} \times  \prod_{i=1}^n\left(\psh{\Delta}\int \psh{\C}\right)_{S_i} \to \psh{\Delta}\int \psh{\C}\] by the rule:
...
\begin{rem} 
	...
	where \(\times^S\) denotes the product in \(\left(\psh{\Delta}\int \psh{\C}\right)_S\).
\end{rem}
...
\begin{thm}[\cite{oury}*{3.78}]
	The category \(\psh{\Delta}\int \psh{\C}\) is cocomplete.
\end{thm}
\begin{proof} 
	...
	\begin{enumerate}[\indent(1)]
		\item The category \(\mathcal{D}\) is cocomplete
	...
	For a general simplicial set \(S\), 
\end{proof}
\begin{note}
	The statement of the previous theorem has a typo.
\end{note}
