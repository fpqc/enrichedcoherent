...
\subsection{Acknowledgements} We would like to thank Denis-Charles Cisinski for his help with axiomatizing the Eilenberg-Zilber shuffle decomposition \ref{cishelp} and George Raptis for several suggested corrections and additions.  We would also like to give special thanks to Andrea Gagna for spending his time discussing these ideas with us over several long conversations as well as his crucial encouragement to put all of this down in writing as well as Eric Peterson for his sage advice and friendship over the years.  We would also like to thank Alexander Campbell for his invaluable help in correcting the proof of Theorem \ref{joyalisothm}.
...
\begin{lemma}\label{precatanodyne}
	...
	\[k_!(A,c) \hookrightarrow k_!(B,c)\] is horizontal inner anodyne. Consequently, if \(X\) is a horizontal \(\C\)-quasicategory, each simplicial set \(k^\ast(X)_c\) is a quasicategory. 
\end{lemma}
\begin{proof}
	...
	\[k_!(\Lambda^n_i,c) \hookrightarrow k_!(\Delta^n,c).\]
\end{proof}

\section{The \(\Hom\) by cosimplicial resolutions for \(\C\)-precategories}
...
\begin{cor}\label{mapspacecor}
	...
	\begin{align*}
		\mathfrak{C}(X)(x,x^\prime)_c \leftrightsquigarrow \Hom(k_!(C^\bullet_R,c), X_{x,x^\prime},
		\intertext{and}
		\mathfrak{C}(X)(x,x^\prime)_c \leftrightsquigarrow \Hom(k_!(C^\bullet_L,c), X_{x,x^\prime}.
	\end{align*}
\end{cor}
We also make note of one more useful fact:
\begin{lemma}\label{resolutionlemma}
	The cosimplicial bi-pointed \(\C\)-cellular sets \(k_!(C^\bullet_R,c)\) and \(k_!(C^\bullet_R,c)\) are Reedy-cofibrant cosimplicial resolutions of \([1](c)=k_!(\Delta^1,c)\) in \(\cellset_{\ast,\ast}\).  
\end{lemma}
\begin{proof}
	...
	\[k_!(C^n_R,c) \to k_!(\Delta^1,c)=[1](c)\]
	and 
	\[k_!(C^n_L,c) \to k_!(\Delta^1,c)=[1](c)\]
...
\end{prop}
\begin{proof}
	By Lemma \ref{resolutionlemma}, we see that we can equivalently compute \(\Map_X(x,y)\) using the cosimplicial resolutions \(k_!(C^\bullet_R,c)\)of \([1](c)\), from which the result follows by Corollary \ref{mapspacecor}.
\end{proof}
