...
\begin{defn} 
	...
	\[
		X_c\mapsto \mathfrak{C}_\Delta(X_c)
	\]
...
\begin{proof} 
	...
	\] 
	which amounts to the data of a natural (in \(c\) and \(n\)) bijection
	\[
		\mathfrak{C}(X)(x, y)_{c,n} \to \mathfrak{C}_{\Delta,\bullet}( k^\ast(X))(x,y)_{c,n}.
	\] 
	Since both functors are cocontinuous, we can reduce further to the case where \(X\) is a representable of the form \([n](c_1,\dots,c_n)\).  
\end{proof}
